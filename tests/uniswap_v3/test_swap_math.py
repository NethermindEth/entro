...
class TestComputeSwapStep:
    def test_exact_amount_in_gets_capped_at_price_target_one_for_zero(
        ...
        pool = initialize_empty_pool(pool_factory=SWAP_MATH_FACTORY)
        ...
        swap_step = pool.math.compute_swap_step(
            price, price_target, liquidity, amount, 600, exact_rounding=True
        )
        ...
        price_after_whole_input_amount = pool.math.get_next_sqrt_price_from_input(
            price, liquidity, amount, False, exact_rounding=True
        )
    ...
    def test_exact_amount_out_gets_capped_at_price_target_one_for_zero(
        ...
        price_after_whole_output_amount = pool.math.get_next_sqrt_price_from_input(
            price, liquidity, amount * -1, False, exact_rounding=True
        )
        assert swap_step.sqrt_price_next == price_target
    ...
    def test_exact_amount_out_fully_recieved_one_for_zero(self, initialize_empty_pool):
        ...
        price_after_whole_output_amount = pool.math.get_next_sqrt_price_from_output(
            price, liquidity, amount * -1, False, exact_rounding=True
        )
    ...
    def test_target_price_of_1_uses_partial_input_amount(self, initialize_empty_pool):
        pool = initialize_empty_pool(pool_factory=SWAP_MATH_FACTORY)
        swap_step = pool.math.compute_swap_step(
            2, 1, 1, 3915081100057732413702495386755767, 1, exact_rounding=True
        )
    ...
    def test_handles_intermediate_insufficient_liqudity_exact_output_zero_for_one(
        ...
        swap_step = pool.math.compute_swap_step(
            sqrt_price, sqrt_price_target, 1024, -4, 3000, exact_rounding=True
        )
    ...
    def test_handes_intermediate_insufficient_liquidity_exact_output_one_for_zero(
        ...
        swap_step = pool.math.compute_swap_step(
            sqrt_price, sqrt_price_target, 1024, -263000, 3000, exact_rounding=True
        )
