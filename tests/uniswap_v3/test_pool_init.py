...
from python_eth_amm.base.token import NULL_TOKEN
from python_eth_amm.exceptions import UniswapV3Revert
from python_eth_amm.math import SqrtPriceMathModule, TickMathModule
...
class TestPoolInitialization:
    ...
    def test_initialization_args_are_passed_to_pool(self):
        pool = self.factory.initialize_empty_pool(
            "uniswap_v3",
            initialization_args={
        ...
        assert pool.initialization_block is None
        assert pool.block_timestamp - datetime.datetime.now().timestamp() < 5

    def test_warning_is_raised_for_fee_tick_spacing_mismatch(self, caplog):
        with caplog.at_level(logging.WARN):
            self.factory.initialize_empty_pool(
                "uniswap_v3",
                initialization_args={
                    "fee": 1000,
                    "tick_spacing": 200,
    ...
    def test_raises_if_initialization_price_too_low(self):
        with pytest.raises(UniswapV3Revert):
            pool = self.factory.initialize_empty_pool(
                "uniswap_v3",
                initialization_args={
                    "initial_price": SqrtPriceMathModule.MIN_SQRT_RATIO - 10,
                },
            )
    ...
    def test_raises_if_initialization_price_too_high(self):
        with pytest.raises(UniswapV3Revert):
            pool = self.factory.initialize_empty_pool(
                "uniswap_v3",
                initialization_args={
                    "initial_price": SqrtPriceMathModule.MAX_SQRT_RATIO + 10,
                },
            )
