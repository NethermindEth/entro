...
def _sort_tokens(a: ChecksumAddress, b: ChecksumAddress) -> tuple[ChecksumAddress, ChecksumAddress]:
    """
...
class PriceOracle:
    ...
    available_markets: dict[tuple[ChecksumAddress, ChecksumAddress], list[TokenMarketInfo]]
    """
    ...
    def update_price_feeds(self):
        ...
        backfilled_price_data = self.db_session.execute(
            select(BackfilledRange).filter(
                BackfilledRange.data_type.in_([BackfillDataType.prices.value, BackfillDataType.spot_prices.value]),
            )
        ).all()
    ...
    def get_price_at_block(self, block_number: int, token_id: ChecksumAddress):
        ...
        ..warning::
            ...
            :py:func:`nethermind.entro.pricing.Oracle.get_price_over_time`
    ...
    def backfill_prices(
        ...
        if token_id in STABLECOIN_ADDRESSES:
            raise OracleError(f"Token {token_id} is a stablecoin, and does not need to be priced.")

        if (not isinstance(start, int) or not isinstance(end, int)) and self.timestamp_converter is None:
            raise OracleError(
        ...
        usdc_pools = self.available_markets.get(_sort_tokens(token_id, USDC_ADDRESS), [])
        if usdc_pools:
            backfill_pool = _filter_markets(usdc_pools)
        else:
            weth_pool = _filter_markets(self.available_markets.get(_sort_tokens(token_id, WETH_ADDRESS), []))
    ...
    def backfill_weth_price(self):
        ...
        usdc_weth_pool = _filter_markets(self.available_markets.get(_sort_tokens(USDC_ADDRESS, WETH_ADDRESS), []))
