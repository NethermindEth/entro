...
def _validate_api_key(key: str | None) -> str:
    ...
    if len(key) != 34:
        raise BackfillError("Etherscan API keys are 34 characters long.  Double check --api-key")
...
def etherscan_backfill_txs(
    ...
    if "for_address" in backfill_plan.filter_params.keys():
        ...
        raise BackfillError("Etherscan transaction backfill only supports --for-address filters or no filters")
...
def backfill_txns_from_account_api(
    ...
    for range_idx, (start_block, end_block) in enumerate(backfill_plan.range_plan.backfill_ranges):
        ...
        while True:
            ...
            if break_loop:
                logger.info(f"Finished Writing Final Batch of {len(raw_tx_batch)} Transactions.  Breaking Loop...")
                break

            blocks_in_batch = int(parsed_transactions[-1].block_number - parsed_transactions[0].block_number)
            search_block = int(parsed_transactions[-1].block_number) + 1

            progress.update(backfill_task, advance=blocks_in_batch, searching_block=search_block)

        if killer.kill_now:
            break
        backfill_plan.range_plan.mark_finalized(range_idx)

    progress.console.print("[green]Finished Backfilling Transactions. Saving Cached Models to DB...")
    writer.finish()
