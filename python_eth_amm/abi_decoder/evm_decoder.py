...
from python_eth_amm.exceptions import DecodingError
...
logger = package_logger.getChild("abi_decoder")
...
class EVMDecoder:
    ...
    def __init__(
        ...
        self.function_decoders = dict(
            [self._load_function_decoder(func) for func in abi_functions]
        )

        self.event_decoders = dict(
            [self._load_event_decoder(event) for event in abi_events]
        )

    def decode_abi_from_types(
        self, types: list[str], data: bytes | bytearray
    ) -> tuple[Any, ...] | None:
        """
        ...
        try:
            ...
            logger.debug(
                f"Insufficient data bytes while decoding {data.hex()} for types {types}"
            )
            return None
        except NonEmptyPaddingBytes:
            logger.debug(
                f"Non-empty padding bytes while decoding {data.hex()} for types {types}"
            )
            return None
        except OverflowError:
            logger.debug(
                f"Overflow error while decoding {data.hex()} for types {types}"
            )
            return None
        except Exception as e:  # pylint: disable=broad-exception-caught
            logger.error(
                f"Unknown error: {e.__class__} while decoding {data.hex()} for types {types}: {e}"
            )
            return None
    ...
    def get_all_decoded_functions(self, full_signature: bool = True) -> list[str]:
        ...
        return [
            decoder.function_signature
            if full_signature
            else signature_to_name(decoder.function_signature)
    ...
    def get_all_decoded_events(self, full_signature: bool = True) -> list[str]:
        ...
        return [
            decoder.event_signature
            if full_signature
            else signature_to_name(decoder.event_signature)
            for decoder in self.event_decoders.values()
        ]

    def _load_function_decoder(
        self, abi_function: ABIFunction
    ) -> tuple[str, FunctionDecoder]:
    ...
    def apply_formatters(
        self, decoding_result: Sequence[Any], types: list[str]
    ) -> list[Any]:
        """
    ...
    def decode_function(
        self, calldata: bytes, transaction_hash: str = ""
        ...
        if function_decoder is None:
            raise DecodingError(
                f"Function with selector {selector} not found in ABI {self.abi_name}"
            )
        ...
        if decoding_result is None:
            logger.debug(
                f"Error Decoding {function_decoder.function_signature} for transaction {transaction_hash}"
            )
            return None
    ...
    def decode_trace(
        ...
        if len(input_data) > 4:
            ...
            if decoded_input is None:
                logger.debug(
                    f"Error Decoding {fn_decoder.function_signature} Input for trace at Tx {transaction_hash}"
                )
                return fn_decoder.function_signature, None, None
            formatted_input = self.apply_formatters(decoded_input, input_types)
            return_input = dict(
                zip(fn_decoder.function_input_names, formatted_input, strict=True)
            )
        else:
            return_input = None
        ...
        if output_data and len(output_data) > 0:
            ...
            return_output = dict(
                zip(fn_decoder.function_output_names, formatted_output, strict=True)
            )
        else:
            return_output = None
    ...
    def decode_event(
        ...
        if event_decoder is None:
            logger.error(
                f"Event with topic {topics[0].hex()} not found in ABI {self.abi_name}"
            )
            raise DecodingError(
                f"Event with topic {topics[0].hex()} not found in ABI {self.abi_name}"
            )
        decoded_data = self.decode_abi_from_types(event_decoder.log_data_types, data)
        decoded_topics = self.decode_abi_from_types(
            event_decoder.log_topic_types, b"".join(topics[1:])
        )

        if decoded_data is None or decoded_topics is None:
            logger.debug(
                f"Error Decoding Event {event_decoder.event_signature} for Transaction {transaction_hash}"
            )
            return None

        formatted_data = self.apply_formatters(
            decoded_data, event_decoder.log_data_types
        )
        formatted_topics = self.apply_formatters(
            decoded_topics, event_decoder.log_topic_types
        )
