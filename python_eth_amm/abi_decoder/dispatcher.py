...
class DecodingDispatcher:
    ...
    def decode_function(
        self, calldata: bytes | str, transaction_hash: str = ""
        ...
        decoding_res = decoder.decode_function(
            calldata=calldata_bytes, transaction_hash=transaction_hash
        )
    ...
    def decode_log(self, log: dict[str, Any]) -> DecodedEvent | None:
        ...
        selector_entry: EVMDecoder | dict[
            int, EVMDecoder
        ] | None = self.event_decoders.get(selector, None)
    ...
    def decode_trace(self, trace_response: dict[str, Any]) -> DecodedTrace | None:
        ...
        return self.decode_trace_data(
            input_data, output_data, trace_response["transactionHash"]
        )
    ...
    def decode_trace_data(
        ...
        function_signature, decoded_input, decoded_output = fn_decoder.decode_trace(
            input_bytes, output_bytes, tx_hash
        )
    ...
    def from_database(
        ...
        if not classify_abis and not all_abis:
            logger.warning(
                "No ABIs loaded into Dispatcher.  Returning empty Dispatcher... "
            )
    ...
    def set_event_decoder(
        ...
        try:
            ...
            if isinstance(existing_decoder, dict):
                raise DecodingError(
                    "Event Signature already loaded with multiple index levels"
                )

            update_index = len(
                existing_decoder.event_decoders[event_selector].log_topic_names
            )
    ...
    def add_function_decoders(self, decoder: EVMDecoder):
        ...
        """
        logger.debug(
            f"Adding {decoder.abi_name} Functions: {', '.join(decoder.get_all_decoded_functions())}"
        )
        for func_selector, func_decoder_dict in decoder.function_decoders.items():
            ...
            if (
                existing_decoder is None
                or existing_decoder.abi_priority < decoder.abi_priority
            ):
                logger.debug(
    ...
    def add_event_decoders(self, decoder: EVMDecoder):
        ...
        """
        logger.debug(
            f"Adding {decoder.abi_name} Events: {', '.join(decoder.get_all_decoded_events())}"
        )
        for event_selector, event_decoder_dict in decoder.event_decoders.items():
            ...
            selector_entry: EVMDecoder | dict[
                int, EVMDecoder
            ] | None = self.event_decoders.get(event_selector, None)

            if selector_entry is None:
                logger.debug(
                    f"Adding event {event_selector} from ABI {decoder.abi_name} to dispatcher"
                )
                ...
                existing_indexed_decoder: EVMDecoder | None = selector_entry.get(
                    add_decoder_index_level, None
                )

                if existing_indexed_decoder is None:
                    self.set_event_decoder(
                        event_selector, add_decoder_index_level, decoder
                    )
                ...
                existing_decoder_index_level = len(
                    selector_entry.event_decoders[event_selector].log_topic_names
                )
    ...
    def decoder_table(
        ...
        sorted_abis = sorted(
            self.loaded_abis.items(), key=lambda x: x[1].abi_priority, reverse=True
        )
