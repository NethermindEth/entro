...
from python_eth_amm.database.writers import EventWriter
from python_eth_amm.exceptions import UniswapV3Revert
from python_eth_amm.tokens import ERC20Token
...
from python_eth_amm.uniswap_v3.math import MAX_TICK, MIN_TICK, UniswapV3Math

package_logger = logging.getLogger("python_eth_amm")
uniswap_logger = package_logger.getChild("uniswap_v3")
logger = uniswap_logger.getChild("chain_interface")
...
def fetch_pool_immutables(
    ...
    token_0 = ERC20Token.from_chain(
        contract.w3, tca(contract.functions.token0().call())
    )
    token_1 = ERC20Token.from_chain(
        contract.w3, tca(contract.functions.token1().call())
    )
...
def fetch_positions(
    ...
    if progress:
        position_task = progress.add_task(
            "Fetching LP Positions", total=len(parsed_keys)
        )
...
def fetch_observations(
    ...
    if progress:
        observation_task = progress.add_task(
            "Fetching TWAP Oracle Observations", total=observation_cardinality
        )
...
def _generate_tick_queue(
    ...
    output_queue = []
    if progress:
        bitmap_task = progress.add_task(
            "Generating Tick Queue from Bitmap", total=upper_bound_key - lower_bound_key
        )
    ...
    logger.info(
        f"Finished Querying Tick Bitmap.  Generated Tick Queue with {len(output_queue)} ticks"
    )
    return output_queue
