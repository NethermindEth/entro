...
from python_eth_amm.exceptions import (
    BackfillError,
    BackfillHostError,
...
logger = backfill_logger.getChild("async")
...
def retry_enabled_batch_post(
    ...
    for retry in range(5):
        if retry == 4:
            logger.error(
                "4th Retry for Same block range. Exiting out of failed backfill"
            )
            return "failed"
        ...
        if any(isinstance(item, BackfillHostError) for item in response):
            logger.error(
                f"Host Error Occurred...  Retrying in 120 seconds...   (Retry Count: {retry + 1})"
            )
            time.sleep(120)
            continue
        ...
        if any(isinstance(item, ServerDisconnectedError) for item in response):
            logger.error(
                f"Server Disconnect Error Occurred...  Retrying in 120 seconds...  (Retry Count: {retry + 1})"
            )
            time.sleep(120)
            continue
...
def _handle_rpc_error(response_json: dict[str, Any]) -> None:
    if "error" in response_json.keys():
        ...
        raise BackfillError(
            "Error in RPC response: " + response_json["error"]["message"]
        )
            ...
            async with session.post(host_address, json=request) as response:
                try:
                    ...
                    match response.status:
                        case 1015 | 429:
                            raise BackfillRateLimitError(
                                "JSON RPC Server Initializing Rate Limits"
                            )
                        ...
                        case _:
                            logger.error(f"\n{'-' * 40}")
                            logger.error(
                                "Unexpected Error in response for request: ", request
                            )
            ...
            async with session.get(host_address, params=request) as response:
                try:
                    ...
                    match response.status:
                        case 1015, 429:
                            raise BackfillRateLimitError(
                                "Server Initializing Rate Limits"
                            )
                        case 500, 502, 503, 504:
                            raise BackfillHostError("Internal Server Error")
                        case _:
