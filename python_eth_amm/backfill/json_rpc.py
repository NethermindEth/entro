...
logger = backfill_logger.getChild("json_rpc")
...
def backfill_block_range(
    ...
    block_writer = ModelWriter(
        db_engine=db_engine, db_model=block_model_for_network(network)
    )
    transaction_writer = (
        ModelWriter(
            db_engine=db_engine,
    ...
    if progress:
        block_task = progress.add_task(
            task_name, total=to_block - from_block, searching_block=from_block
        )
    ...
    while search_block <= to_block:
        ...
        if progress:
            progress.update(
                block_task, advance=batch_size, searching_block=search_block
            )
        ...
        try:
            ...
            logger.error(
                f"Failed to fetch blocks ({search_block} - {min(to_block, search_block + batch_size)})"
            )
            if killer:
...
def get_block_models_for_range(
    ...
    if block_json == "failed":
        logger.error(
            f"Querying Blocks from RPC failed between ({from_block} - {to_block - 1})"
        )
        raise BackfillError()
...
def get_transaction_receipts_for_txns(
    ...
    enriched_txns = add_receipt_to_tx_models(
        transactions=txns, receipt_responses=receipt_json, strict=True
    )
...
def cli_get_full_blocks(
    ...
    for range_idx, (from_block, to_block) in enumerate(backfill_plan.block_ranges):
        ...
        while search_block <= to_block:
            ...
            progress.update(
                backfill_task, advance=batch_size, searching_block=search_block
            )
            ...
            try:
                block_models, intermediate_tx_models = get_block_models_for_range(
                    ...
                    db_dialect=block_writer.db_dialect,
                    max_concurrency=backfill_plan.metadata_dict.get(
                        "max_concurrency", 50
                    ),
                )
            except BackfillError:
...
def cli_get_logs(  # pylint: disable=too-many-branches
    ...
    for range_idx, (start_block, end_block) in enumerate(backfill_plan.block_ranges):
        ...
        if isinstance(backfill_res, int):
            logger.error(
                f"Backfilled up to Block {backfill_res}.  Terminating Backfill & "
                f"Updating Backfilled Range in DB..."
            )
