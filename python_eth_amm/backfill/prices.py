...
from python_eth_amm.cli.utils import progress_defaults
from python_eth_amm.database.models.prices import (
    SUPPORTED_POOL_CREATION_EVENTS,
    MarketSpotPrice,
)
from python_eth_amm.database.readers.prices import get_pool_creation_backfills
from python_eth_amm.exceptions import BackfillError, OracleError
...
def get_price_events_for_range(
    ...
    if event_responses == "failed":
        raise BackfillError(
            f"Failed to Backfill Price Events Between ({start} - {end})"
        )
    ...
    for range_idx, (start_block, end_block) in enumerate(
        ...
        for start_slice in range(start_block, end_block, batch_size):
            ...
            if killer and killer.kill_now:
                logger.warning(
                    f"[red]Processing Terminated Backfill up to block {start_slice}"
                )
                return
            ...
            sorted_events = sorted(
                pricing_events, key=lambda x: (x["block_number"], x["log_index"])
            )
                ...
                if prev_event["block_number"] != event["block_number"]:
                    spot_price_models.append(
                        MarketSpotPrice(
                            ...
                            spot_price=translator.decode_price_from_event(
                                prev_event, ref_token
                            ),
                        )
                    )
