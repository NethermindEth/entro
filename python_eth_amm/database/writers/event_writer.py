...
from python_eth_amm.abi_decoder import DecodedEvent
from python_eth_amm.abi_decoder.utils import signature_to_name
from python_eth_amm.database.models import default_event_model_for_network
from python_eth_amm.database.models.uniswap import UNI_EVENT_MODELS
from python_eth_amm.database.writers.utils import db_encode_dict
from python_eth_amm.types.backfill import SupportedNetwork
from python_eth_amm.utils import camel_to_snake, maybe_hex_to_int
...
logger = database_logger.getChild("event_writer")
...
class EventWriter(BaseWriter):
    ...
    def _db_encode_event(
        self, decoded_event: dict[str, Any], snake: bool = False
    ) -> dict[str, Any]:
        db_encoded_event = {}
    ...
    def finish(self):
        ...
        logger.info(
            f"Wrote {self.total_rows_written} Events to database in {minutes} minutes {seconds:.1f} seconds"
        )
