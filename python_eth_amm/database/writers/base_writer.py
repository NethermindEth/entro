...
logger = database_logger.getChild("base_writer")
...
class BaseWriter:
    ...
    def save_models(
        self, model_data: Sequence[DeclarativeBase], insert_table: TableClause
    ):
        """
        ...
        if self.db_dialect == "postgresql":
            statement = postgresql.insert(insert_table).values(
                [model_to_dict(row) for row in model_data]
            )
