...
logger = database_logger.getChild("model_writer")
    ...
    def add_backfill_data(self, data: Sequence[DeclarativeBase]):
        ...
        for row in data:
            if not isinstance(row, self.db_model):
                logger.error(
                    f"Passed model type: {type(row)} to ModelWriter configured for {self.db_model}"
                )
        ...
        if self.save_to_db:
            ...
            if len(self.db_cache) > 500:
                logger.info(
                    f"Writing {len(self.db_cache)} {self.db_table.name} models to DB"
                )
                logger.debug(
