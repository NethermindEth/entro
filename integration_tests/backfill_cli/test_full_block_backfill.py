...
from python_eth_amm.database.models.ethereum import Block, DefaultEvent, Transaction
from python_eth_amm.database.writers.utils import db_encode_dict, model_to_dict
...
def test_backfill_mainnet_full_block(
    ...
    mig_result = runner.invoke(cli_entry_point, ["migrate-up", *cli_db_url])
    ...
    assert (
        events[-1].decoded_event["sender"]
        == "0x3fC91A3afd70395Cd496C647d5a6CC9D4B2b7FAD"
    )
    assert (
        events[-1].decoded_event["to"] == "0xb9e4345182bE80E45a5E649367F25ffB0cD9Aed3"
    )
