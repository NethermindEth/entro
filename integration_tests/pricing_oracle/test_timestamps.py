...
def test_timestamp_conversions(eth_rpc_url, integration_db_session, integration_postgres_db):
    migrate_up(integration_db_session.get_bind())
    ...
    assert timestamp_converter.block_to_datetime(0) == datetime(2015, 7, 30, 15, 26, 28, tzinfo=timezone.utc)

    assert timestamp_converter.block_to_datetime(18_000_000) == datetime(2023, 8, 26, 16, 21, 35, tzinfo=timezone.utc)
    ...
    assert (
        abs(timestamp_converter.datetime_to_block(datetime(2022, 12, 18, 8, 21, 35, tzinfo=timezone.utc)) - 16210345)
        <= 5
    )
